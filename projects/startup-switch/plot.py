...
VOLTAGE_OFFSET = 0 # Internal voltage offset of the TMCS1108A3U current sensor, derived from csv data
...
def plot_voltage_and_current(filename, sensitivity_mv_per_a=200, time_min=None, time_max=None, output_filename=None, show_smoothed=True):
    ...
    # Extract test number from filename (e.g., TEK0000.CSV -> 0)
    test_number = ''.join(filter(str.isdigit, os.path.basename(filename)))
    ...
    # Add test number in top left corner (use same test_number from above)
